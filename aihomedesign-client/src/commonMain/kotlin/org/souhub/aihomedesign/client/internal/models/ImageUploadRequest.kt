...
internal class ImageUploadRequest(
    val image: ByteArray,
    val contentType: ContentType = detectMimeType(image),
) {
    companion object {
        private fun detectMimeType(b: ByteArray): ContentType {
            ...
            ) return ContentType.Companion.parse("image/webp")
            ...
            ) return ContentType.Companion.parse("image/tiff")

            if (b.size >= 3 && b[0] == 'F'.code.toByte() && b[1] == 'U'.code.toByte() && b[2] == 'J'.code.toByte())
                return ContentType.Companion.parse("image/x-fuji-raf")

            return ContentType.Companion.parse("application/octet-stream")
        }
    }
}